...
./hpa-test/run-k6-testrun.sh platform/k6-hpa-test/testrun-templates/photo-only.yaml
```
...
kubectl -n k6 logs -f -l k6_cr=photo-only --max-log-requests=20 --tail=50
...
### 전체 목적
photo 경로를 **GET 중심으로 단독 부하**하여 photo/gateway의 병목과 스케일링 반응을 관찰합니다.
...
- **대상**:
  - `PHOTO_URL=http://service-gateway.gateway.svc.cluster.local/photo/` (기본: gateway 경유)
  - `HOST_HEADER=api.yongun.shop`
- **GET 경로(스크립트 기본값)**:
  - `PHOTO_GET_PATH=health` → 최종 GET URL은 `PHOTO_URL + PHOTO_GET_PATH`
  - 필요하면 `PHOTO_GET_URL`로 완전한 URL을 직접 지정 가능

> 값을 바꾸려면 `platform/k6-hpa-test/testrun-templates/photo-only.yaml`을 복사해서 숫자/URL만 수정 후 apply 하세요.

### 2) 동작 흐름
1. 각 VU는 1초 페이싱 루프에서 매초 `GETS_PER_SEC` 기대치만큼 GET을 수행합니다.
